...
def main():
    ...
    files = ['ycsb_aegonkv', 'ycsb_diffkv', 'ycsb_blobdb', 'ycsb_titan', 'ycsb_titan', 'ycsb_rocksdb']
    ...
    for workload in workloads:
        ...
        for idx in range(len(files)):
            ...
            with open(result_path, 'r') as f:
                ...
                latency_temp = re.findall(r'200000000 operations.+', content)
                ...
                cur_compaction.append((int(compaction_read_match[1][len('rocksdb.compact.read.bytes COUNT : '):]) + int(
                    compaction_write_match[1][len('rocksdb.compact.write.bytes COUNT : '):])) // 1000000000)

                stall_match = re.findall(r'rocksdb.stall.micros COUNT : \d+', content)
                cur_stall.append(stall_match[1][len('rocksdb.stall.micros COUNT : '):])
